...
const BookingPage = async () => {
  const payments = (await getPayments()) as unknown as PaymentResponse;
    ...
    data.map((item: any, index: number) => {
      contents.push([
        { value: "#" + item.booking.id },
        {
          value: (
            <Image
              src={getImage(item?.booking?.tour?.images[0]?.thumbnail) || ""}
              width={100}
              height={100}
              alt="tour image"
        ...
        { value: item.booking.tour.title },
        { value: moment(item.booking.booking_date).format("DD MMM, YYYY") },
        { value: item.amount },
        { value: item.status === 1 ? "Paid" : "Unpaid" },
        { value: <ButtonSecondary href={`/my-account/bookings/${item.id}`}>View</ButtonSecondary> },
      ]);
    });
  }
      ...
      <div className="flex flex-col">
        <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            ...
            {data.length > 0 && (
              <Pagination pagination={meta} pageUrl="/my-account/bookings" />
            )}
          </div>
        </div>
      </div>
